...
const ghosts = {
  spirit: {
    label: "Spirit",
    description:
      "A Spirit is the most common Ghost you will ever come across. However, it is still very powerful and dangerous. They are usually discovered at one of their hunting grounds after an unexplained death.",
    strength: "Nothing.",
    weakness:
...
function getEvidenceProbability(activeGhosts, evidence, currentEvidenceStatus) {
  if(evidence.status !== possibilities.POSSIBLE){
    return evidence.status === possibilities.CONFIRMED ? 1 : 0;
  };

  const ghostsWithEvidence = Object.keys(activeGhosts).map(
      (ghost, ghostStatus) => ghost.evidence.includes(evidence)
    ).length;

  return ghostsWithEvidence / activeGhosts.length;
};
